...
"Our headline metric, Spearman rank correlation between predicted and realized next-year returns, averages near zero and swings between −0.17 and +0.22 across test years — at 40 stocks per year, that whole range is statistically indistinguishable from no signal. So instead of point forecasts we show rankings with confidence components, flag inference-only rows, and down-weight our own scores for the weak backtest (the scoring code applies an explicit penalty for it)."
