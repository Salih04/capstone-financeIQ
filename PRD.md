...
- **Capstone status: complete** (see `TASK_STATE.md`). Current root/backend suite counts and data-validation results are recorded once in `docs/VERIFICATION_BASELINE.md`; both suites are green. The stale `call_local_llm` references were resolved by OPS-01.
...
4. **Frontend**: 23 `*Page.jsx` files exist in `frontend/src/pages/`. `frontend/src/App.jsx` declares 27 `<Route>` entries: 22 render page components and 5 are redirects. Core research routes include `/dashboard`, `/research-agent`, `/companies`, `/experiments`, `/research`, `/data-quality`, `/benchmark`, and `/forecasting`; demo data remains fallback-only, and research surfaces keep caveats and IC ≈ 0 visible.
...
- **Test baseline ownership.** Current observed suite counts are maintained in `docs/VERIFICATION_BASELINE.md`; historical counts are not repeated here.
