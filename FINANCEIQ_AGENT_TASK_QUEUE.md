...
**Universal verification (updated 2026-07-12):** after backend edits run `PYTHONPATH=backend python -m pytest backend/tests` (expect **54 pass**, or more if your task adds tests); after pipeline/test edits run `PYTHONPATH=. python -m pytest tests/` (expect **97 pass**, or more); after data edits also run `make data-validate` (expect VALID — 403 rows, 40 features, 321 target rows); after any user-facing copy or response-constant change run `make claims-lint` (expect exit 0). Rollback for all tasks: `git checkout -- <files>` before commit, `git revert` after — no task below has irreversible side effects unless its rollback note says otherwise.
